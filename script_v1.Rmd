...
To have a perspective of how different the energy consumption for each household is, I plot the energy consumption along the y-axis against time from past to future. As can be observed from this animation, energy consumption in households vary substantially, which is a reflection of their varied behaviors. We could hardly interpret the raw data for these individual households when plotted like these as they are all squeezed. But if we zoom in and try to look closely at the individual hoseholds patterns emerge. In most cases, this data will have multiple seasonal patterns like days of the week where, for example, more electricity will be consumed for a working day and less for non-working day or hours of the day since some families will have night owls while others will have morning larks or across months of the year since different households use different heating or cooling methods. Deconstructing time in several possible ways can help us understand the different seasonal or periodic patterns in the data. Iteratively exploring all possible time deconstructions for comprehending possible periodicities could be overwhelming and thus a systematic way to do so might be helpful.


## Slide 4

But when I want to restructure time from linear to periodic, the structure of the data gets re-organized such that there are several data points for levels of the time deconstructions. For example, when I see the raw data across days of the week even for one household, I will have several points for each of Sunday, Monday and so on. So summarisation of this data is important without losing key features of the data. But aggregate or just mean and median hides the distributions of usage at finer scales and could be misguiding. Because of the large volume of data available, using displays of probability distributions conditional on one or more granularities is a potentially useful approach.

Visualizing probability distributions across any time deconstructions that accomodate for periodic behavior can help comprehend and compare the seasonal pattern or anomalies in behavior across few households.

However, when we have large number of households to analyze, we need to move on to clustering these households to explore similar pockets of behavior and then examine for any seasonal patterns or irregularities.
...
Aperiodic time granularities are the ones which can not be specified as a periodic repetition of a pattern of granules. Most public holidays repeat every year, but there is no finite (or reasonably small) period within which their behavior remains constant. A classic example can be that of Easter, whose dates repeat only after 5,700,000 years. Another example could be semester holidays which include summer holidays, winter holidays, mid semester breaks and Swot Vac. So if someone asks which is the 5th day of the Swot Vac., I cannot use the periodic characteristics of time to define it. In that case, using a proxy label for each kind of breaks corresponding to the linear holidays seems to be the best approach.

Now using calendar algebra we can also generate multiple order up singularities from single-order-up granularities and vice versa.
